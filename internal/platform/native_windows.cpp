...
static void closeObject(IUnknown *object) {
    ...
    if (SUCCEEDED(object->QueryInterface(iidClosable, reinterpret_cast<void **>(closable.put()))))
        closable->Close();
}
...
static int fail(char *error, size_t size, const char *message, HRESULT status = S_OK) {
    if (FAILED(status))
        snprintf(error, size, "%s (HRESULT 0x%08lx)", message, static_cast<unsigned long>(status));
    else
...
static HRESULT factory(const wchar_t *name, REFIID iid, void **result) {
    ...
    if (SUCCEEDED(hr))
        hr = RoGetActivationFactory(text, iid, result);
    if (text)
...
static BOOL CALLBACK enumerateWindow(HWND window, LPARAM param) {
    ...
    DWORD pid = 0;
    if (!eligible(window, &pid))
        return TRUE;
    RECT rect{};
    if (!GetWindowRect(window, &rect) || rect.right - rect.left < 100 ||
    ...
    if (node)
        node->executable = static_cast<char *>(calloc(bytes + 1, 1));
    if (!node || !node->executable) {
        free(node);
        list.allocationFailed = true;
        return FALSE;
    }
...
fdb_win_window *fdb_win_windows(char *error, size_t size) {
    ...
    if (!EnumWindows(enumerateWindow, reinterpret_cast<LPARAM>(&list))) {
        ...
        fail(error, size,
             list.allocationFailed ? "Could not allocate game window metadata"
                                   : "Could not enumerate application windows");
        return nullptr;
    }
    return list.head;
}
...
static void updateCaptureBorder(fdb_wgc *c) {
    ...
    if (!c->borderRequested) {
        ...
        HRESULT hr = factory(L"Windows.Graphics.Capture.GraphicsCaptureAccess", iidCaptureAccess,
                             reinterpret_cast<void **>(access.put()));
        if (SUCCEEDED(hr))
            hr = access->RequestAccessAsync(fdbAccessBorderless, c->borderAccess.put());
        if (SUCCEEDED(hr))
            hr = c->borderAccess->QueryInterface(
                iidAsyncInfo, reinterpret_cast<void **>(c->borderAccessInfo.put()));
        if (FAILED(hr)) {
            c->borderAccess.reset();
            c->borderAccessInfo.reset();
        }
    }
    if (c->borderAccessInfo.p) {
        AsyncStatus status = Started;
        HRESULT hr = c->borderAccessInfo->get_Status(&status);
        if (SUCCEEDED(hr) && status == Started)
            return;
        INT32 result = 0;
        c->borderAllowed = SUCCEEDED(hr) && status == Completed &&
                           SUCCEEDED(c->borderAccess->GetResults(&result)) &&
                           result == fdbAccessAllowed;
        if (FAILED(hr))
            c->borderAccessInfo->Cancel();
        c->borderAccessInfo->Close();
        c->borderAccessInfo.reset();
        c->borderAccess.reset();
    }
    if (c->borderAllowed)
        c->borderSession->SetBorderRequired(false);
...
fdb_wgc *fdb_wgc_open(char *error, size_t size) {
    ...
    if (FAILED(hr)) {
        fail(error, size, "Initialize Windows Runtime", hr);
        return nullptr;
    }
    ...
    if (SUCCEEDED(hr))
        hr = factory(L"Windows.Graphics.Capture.Direct3D11CaptureFramePool", iidPoolStatics2,
                     reinterpret_cast<void **>(c->poolFactory.put()));
    if (FAILED(hr)) {
        fail(error, size, "Window capture requires Windows 10 version 1903 or newer", hr);
        fdb_wgc_close(c);
        return nullptr;
    }
    hr = D3D11CreateDevice(nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr,
                           D3D11_CREATE_DEVICE_BGRA_SUPPORT, nullptr, 0, D3D11_SDK_VERSION,
                           c->device.put(), nullptr, c->context.put());
    if (FAILED(hr)) {
        fail(error, size, "Create Direct3D capture device", hr);
        fdb_wgc_close(c);
        return nullptr;
    }
    ...
    if (SUCCEEDED(hr))
        hr = CreateDirect3D11DeviceFromDXGIDevice(dxgi.p, inspectable.put());
    if (SUCCEEDED(hr))
        hr = inspectable->QueryInterface(iidRTDevice, reinterpret_cast<void **>(c->rtDevice.put()));
    if (FAILED(hr)) {
        fail(error, size, "Create WinRT capture device", hr);
        fdb_wgc_close(c);
        return nullptr;
    }
    return c;
}
void fdb_wgc_close(fdb_wgc *c) {
    delete c;
    RoUninitialize();
}
...
int fdb_wgc_select(fdb_wgc *c, uintptr_t id, uint32_t pid, int *width, int *height, char *error,
    ...
    if (!eligible(window, &owner) || owner != pid) {
        fdb_wgc_reset(c);
        return fail(error, size, "Game window is no longer available");
    }
    ComPtr<FdbCaptureItem> item;
    // The only source constructor in this backend takes a specific HWND.
    HRESULT hr =
        c->itemFactory->CreateForWindow(window, iidItem, reinterpret_cast<void **>(item.put()));
    FdbSize dimensions{};
    if (SUCCEEDED(hr))
        hr = item->Size(&dimensions);
    if (FAILED(hr)) {
        fdb_wgc_reset(c);
        return fail(error, size, "Select game window", hr);
    }
    if (dimensions.width <= 0 || dimensions.height <= 0 ||
        int64_t(dimensions.width) * dimensions.height > 100000000) {
        fdb_wgc_reset(c);
        return fail(error, size, "Invalid game window dimensions");
    }
    *width = dimensions.width;
    *height = dimensions.height;
    if (c->window == window && c->pid == pid && c->content.width == dimensions.width &&
        c->content.height == dimensions.height)
        return 0;
    fdb_wgc_reset(c);
    c->item.reset(item.detach());
    c->content = dimensions;
    hr = c->poolFactory->CreateFreeThreaded(c->rtDevice.p, DXGI_FORMAT_B8G8R8A8_UNORM, 2,
                                            dimensions, c->pool.put());
    if (SUCCEEDED(hr))
        hr = c->pool->CreateCaptureSession(c->item.p, c->session.put());
    if (SUCCEEDED(hr)) {
        ComPtr<FdbSession2> cursor;
        if (SUCCEEDED(
                c->session->QueryInterface(iidSession2, reinterpret_cast<void **>(cursor.put()))))
    ...
    if (FAILED(hr)) {
        fdb_wgc_reset(c);
        return fail(error, size, "Start game window capture", hr);
    }
    c->window = window;
    c->pid = pid;
    return 0;
...
static HRESULT cacheFrame(fdb_wgc *c, FdbFrame *frame) {
    ...
    if (FAILED(hr))
        return hr;
    if (dimensions.width != c->content.width || dimensions.height != c->content.height)
    ...
    if (SUCCEEDED(hr))
        hr = surface->QueryInterface(iidDxgiAccess, reinterpret_cast<void **>(access.put()));
    if (SUCCEEDED(hr))
        hr = access->GetInterface(IID_ID3D11Texture2D, reinterpret_cast<void **>(texture.put()));
    if (FAILED(hr))
        return hr;
    D3D11_TEXTURE2D_DESC desc{};
    texture->GetDesc(&desc);
    if (desc.Format != DXGI_FORMAT_B8G8R8A8_UNORM || desc.Width < UINT(dimensions.width) ||
        desc.Height < UINT(dimensions.height))
        return E_FAIL;
    D3D11_TEXTURE2D_DESC previous{};
    if (c->latest.p)
        c->latest->GetDesc(&previous);
...
int fdb_wgc_capture(fdb_wgc *c, int x, int y, int width, int height, void *rgba, char *error,
    ...
    if (x < 0 || y < 0 || width <= 0 || height <= 0 || int64_t(x) + width > c->content.width ||
        int64_t(y) + height > c->content.height)
    ...
    do {
        ...
        if (FAILED(hr)) {
            fdb_wgc_reset(c);
            return fail(error, size, "Read game window frame", hr);
        }
        if (frame.p) {
            ...
            if (FAILED(hr)) {
                ...
                return fail(error, size,
                            "Game frame changed or graphics device failed; reacquiring window", hr);
            }
            if (++received < 4)
                continue;
        }
        if (c->latest.p)
            break; // Static game content need not generate new frames.
        Sleep(5);
    } while (GetTickCount64() < deadline);
    if (!c->latest.p)
        return fail(error, size,
                    "No game frames yet; restore WoW and use windowed or borderless mode");
    D3D11_TEXTURE2D_DESC desc{};
    if (c->staging.p)
        c->staging->GetDesc(&desc);
    HRESULT hr = S_OK;
    ...
    if (FAILED(hr))
        return fail(error, size, "Allocate game frame readback", hr);
    ...
    if (FAILED(hr)) {
        fdb_wgc_reset(c);
        return fail(error, size, "Read game pixels", hr);
    }
