...
struct StateBufferIO {
    ...
    void restore_snapshot(const Snapshot& snap) {
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            bool sliced = !is_dynamic[i] && (snap.shapes[i] != init_shapes[i]) && (has_f32 || has_f16);
        ...
        shapes = init_shapes;
        for (size_t i = 0; i < n; ++i) if (is_dynamic[i]) shapes[i] = snap.shapes[i];
    }
...
void PocketTTS::stream(const std::string& text, const Tensor& voice, StreamCallback cb, int max_frames) {
    ...
    for (size_t si = 0; si < sentences.size(); ++si) {
        ...
        std::thread gen_thread([&]() {
            ...
            } catch (...) { gen_error = std::current_exception(); }
            {
                std::lock_guard<std::mutex> lock(mtx);
                gen_done = true;
            }
