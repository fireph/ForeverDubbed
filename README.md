...
**Upgrading to 0.3.1:** replace both addon and companion, then `/reload`. This adds NPC race/gender/ID metadata and Pocket TTS as the default speech backend. Version 0.3.1 fixes missing NPC races using confirmed NPC identities and a character-model fallback, and adds Skyborne, Goblin, Blood Elf, and Draenei voice profiles. Run the setup once and use `Start-ForeverDubbed.cmd`. A 0.2.1 addon still decodes in the new reader, but has no metadata for race voices.
...
**Compatibility status:** the manifest targets Forever interface **16001**. The 16-color, 2-pixel transport has been confirmed working in-game. The Pocket TTS integration has been tested on Windows with real CPU synthesis and Go audio playback. Race extraction, including asynchronous model lookup, is covered by Lua API doubles; model-based identification should still be checked in-game with `/fdb npc`; NPCs whose race is unavailable use the configured fallback. If a later beta marks the addon out of date, inspect `/fdb status` before updating the TOC.
...
| `/fdb npc` | Inspect the dialog/target NPC: race, source, gender, ID, and model file ID |
...
Selection order is `-voice`, `npc_overrides`, race/gender, then `default`. Each race has an explicit `unknown` gender fallback. Other races and unavailable identities use `narrator_male` (`jean`) for male or unknown gender, and `human_female` (`alba`) for female speakers. The narrator fallback is separate from the Human male profile, so you can change it independently. For example, `"npc_overrides": {"4949": "orc_male"}` assigns that NPC the Orc male profile. The JSON output includes `race`, `gender`, and `npc_id` when available.

`UnitRace` frequently returns nil for NPCs even when their gender and GUID are available. The addon first uses saved NPC race assignments, public API race information, and confirmed NPC-ID mappings. Zephras Citizen (254100) is mapped to Skyborne based on in-game confirmation. Unknown NPCs are probed with an invisible `PlayerModel`, matching known character FileDataIDs from the [WoW file list](https://github.com/wowdev/wow-listfile). This uses documented [SetUnit](https://github.com/Gethe/wow-ui-source/blob/forever/Interface/AddOns/Blizzard_APIDocumentationGenerated/FrameAPICharacterModelBaseDocumentation.lua) and [GetModelFileID](https://github.com/Gethe/wow-ui-source/blob/forever/Interface/AddOns/Blizzard_APIDocumentationGenerated/SimpleModelAPIDocumentation.lua) APIs. Model loads get up to 600 ms before the text is sent; a newer message cancels publication of an older pending message.
